...
if __name__ == '__main__':
    ...
    while date_value < datetime.datetime.now().date():
        try:
            ...
            vca_import = vca_handler.read_vca(config.import_path, config.extension, date_value, date_value, **config.import_tag)
            vca_export = vca_handler.read_vca_by_job(config.export_path, vca_import.keys(), config.extension, 0, 12, **config.export_tag)
